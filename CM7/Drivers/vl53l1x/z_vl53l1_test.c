...
// test can be done in two modes depending if using interrupt or polling mode
	...
	while ((!HAL_GPIO_ReadPin(TOF_GPIO_Port, TOF_GPIO_Pin)) && ((HAL_GetTick()-testingTime)<=VL53L1__INTERMEASUREMENT)) {};
	if (HAL_GPIO_ReadPin(TOF_GPIO_Port, TOF_GPIO_Pin)) {
#else
	uint8_t dataReady=0;
